 ...
 */
public class Order_four_adapter extends BaseAdapter {
    private List<item_quanbu> list;
    Context context;

    public Order_four_adapter(Context context, List<item_quanbu> list) {
        this.list = list;
    ...
    boolean flag_complete = false;//开始工作
    boolean flag_reject = false;//申请拒绝
    boolean flag_ok = false;//完成
    boolean flag_quxiao = false;//已经取消但是没支付
    boolean flag_tuikuan = false;//已退款
    boolean flag_pingjia = false;//已经评价
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        view = LayoutInflater.from(context).inflate(R.layout.order_quanbu, null);
        ...
        View overtime = view.findViewById(R.id.overtime);
...
//                    overtime_flag=true;
        ...
        if (flag_quxiao || flag_tuikuan || flag_pingjia) {
...
//                                                                }
                ...
                if (flag_ok) {
                    ...
                    System.out.println("youmeiyou有子订单");
                    //不是试用订单
                    //有子订单
                    if (Integer.parseInt(list.get(position).getService_type_id()) > 8) {
                        go_pay.setVisibility(View.VISIBLE);
                                                                ...
                                                                if (response.getString("ret").equals("200") && response.getJSONObject("data").getString("status").equals("1")) {
                                                                    ...
                                                                    view.findViewById(R.id.ok_btn).setOnClickListener(new View.OnClickListener() {
                                                                        ...
                                                                        public void onClick(View v) {
                                                                            alert.dismiss();
                                                                            if (list.get(position).getPayed().equals("0")) {
                                                                                list.get(position).setStatus("5");
                                                                            } else {
                                                                                list.get(position).setStatus("6");
                                                                            }
