...
public class Order_pay_tc extends TempletActivity<MyInfoStore, PayActionsCreator> {
    ...
    private void init_text() {
        service_place.setText(getIntent().getStringExtra("contact_addr")+getIntent().getStringExtra("contact_door"));
        service_user.setText(getIntent().getStringExtra("contacts"));
        service_phone.setText(getIntent().getStringExtra("contact_phone"));
        service_price.setText(getIntent().getStringExtra("price"));
        tcv_title.setText(getIntent().getStringExtra("ccsp_title"));
        tc_pri.setText(getIntent().getStringExtra("price"));
    ...
    private void init_coupon_click() {
        coupon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(Order_pay_tc.this, Coupon.class);
                intent.putExtra("type_num",getIntent().getStringExtra("type_id"));
                intent.putExtra("areaid",areaid);
                System.out.println("需要传的type_num:"+getIntent().getStringExtra("type_id"));
            ...
            public void onClick(View v) {
                ...
                switch (pay_way){
                    case 1://余额
                        ...
                        break;
                    case 2://微信
                        if (isWeixinAvilible(Order_pay_tc.this)){
                            weixin_pay();
                        }else {
                ...
                try {
                    ...
                    Runnable payRunnable = new Runnable() {
                        ...
                        public void run() {
                                ...
                                PayTask alipay = new PayTask(Order_pay_tc.this);
            ...
            switch (msg.what) {
                ...
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        Toast.makeText(Order_pay_tc.this, "支付成功", Toast.LENGTH_SHORT).show();
                        Intent intent=new Intent(Order_pay_tc.this, MainActivity.class);
...
//                        actionsCreator().customerPay(totalfee, paynumber, AyiApplication.getInstance().accountService().id(), AyiApplication.getInstance().accountService().token(), RechargeActivity.this);
                        ...
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(Order_pay_tc.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
                        } else if (TextUtils.equals(resultStatus, "4000")){
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(Order_pay_tc.this, "支付失败", Toast.LENGTH_SHORT).show();
...
////                        finish();
                        ...
                        Toast.makeText(Order_pay_tc.this,"调用支付宝失败，请重试",Toast.LENGTH_SHORT).show();
                    }
...
//                        diaoyong_weix("0.01");
                        ...
                        progressBar1.setVisibility(View.GONE);
                        button_home_zdg_ok_btn.setEnabled(true);
                        Toast.makeText(Order_pay_tc.this,jsonObject.getString("msg"),Toast.LENGTH_SHORT).show();
                    }
...
//        actionsCreator().weixinPay(body, totalfee, goodstag, source, AyiApplication.getInstance().accountService().id(), AyiApplication.getInstance().accountService().token(), Order_pay.this);
    ...
    public void weixinCallback(WeixinInfo weixinInfo) {
        ...
        actionsCreator().weixinCallback(weixinInfo.getOut_trade_no(), "APP", AyiApplication.getInstance().accountService().id(), AyiApplication.getInstance().accountService().token(), Order_pay_tc.this);
    ...
    public BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (str.equals(RetrofitUtil.APP_BORADCASTRECEIVER2)) {
                ...
                WeiXin.createWXAPI(weixinInfo, Order_pay_tc.this);//发起微信
...
//                                            finish();
                            ...
                            progressBar1.setVisibility(View.GONE);
                            button_home_zdg_ok_btn.setEnabled(true);
                            Toast.makeText(Order_pay_tc.this,jsonObject.getJSONObject("data").getString("message"),Toast.LENGTH_SHORT).show();
                        }
                    }else{
                        progressBar1.setVisibility(View.GONE);
                        button_home_zdg_ok_btn.setEnabled(true);
            ...
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                ...
                Toast.makeText(Order_pay_tc.this,responseString,Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void init() {
        ...
        tc_pri= (TextView) findViewById(R.id.tc_pri);
        tcv_title= (TextView) findViewById(R.id.tcv_title);
